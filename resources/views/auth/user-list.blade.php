                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td class="min-w-[150px] text-black dark:text-white ng-tns-c19-2">
                                <div class="flex items-center ng-tns-c19-2">
                                    @if ($user->profile_photo_path)
                                        <img src="{{ asset('/storage/' . $user->profile_photo_path ) }}" alt="avatar" class="h-8 w-8 rounded-md object-cover ltr:mr-3 rtl:ml-3 ng-tns-c19-2">
                                        <span class="whitespace-nowrap ng-tns-c19-2 ml-2">{{ $user->name }}</span>
                                    @else
                                        <img src="{{ $user->profile_photo_url }}" alt="avatar" class="h-8 w-8 rounded-md object-cover ltr:mr-3 rtl:ml-3 ng-tns-c19-2">
                                        <span class="whitespace-nowrap ng-tns-c19-2 ml-2">{{ $user->name }}</span>
                                    @endif
                                </div>
                            </td>
                            ...
                            {{-- <td class="border px-4 py-2 text-center">
                                <div class="flex justify-center">
                                    <a href="{{ route('user.edit', $user->id) }}" class="bg-violet-500 dark:bg-violet-700 hover:bg-violet-600 dark:hover:bg-violet-800 text-white font-bold py-2 px-4 rounded mr-2">Edit</a>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-center">
                                    <!-- Modal toggle -->
                                    <button data-modal-target="viewUserModal" data-modal-toggle="viewUserModal" data-user-id="{{ $user->id }}" class="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
                                        
                                        <svg data-user-id="{{ $user->id }}" class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                            <path data-user-id="{{ $user->id }}" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 9h3m-3 3h3m-3 3h3m-6 1c-.306-.613-.933-1-1.618-1H7.618c-.685 0-1.312.387-1.618 1M4 5h16a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Zm7 5a2 2 0 1 1-4 0 2 2 0 0 1 4 0Z"/>
                                        </svg>
                                    </button>
                                    {{-- <a href="{{ route('users.index', $user->id) }}" class="bg-violet-500 dark:bg-violet-700 hover:bg-violet-600 dark:hover:bg-violet-800 text-white font-bold py-2 px-4 rounded mr-2">
                                    </a> --}}
                                </div>                                
                            </td>
